...
@section('halaman', 'Edit Rute Perjalanan')
            ...
            <form action="{{ route('rute.update', $rute->id_rute) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control js-example-basic-single @error('lokasi_asal') is-invalid @enderror" name="lokasi_asal">
                        ...
                        @forelse($wisata as $item)
                            <option value="{{ $item->id_wisata }}" {{ old('lokasi_asal', $rute->lokasi_asal) == $item->id_wisata ? 'selected' : '' }}>{{ $item->nama_wisata }}</option>
                        @empty
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control js-example-basic-single @error('lokasi_tujuan') is-invalid @enderror" name="lokasi_tujuan">
                        ...
                        @forelse($wisata as $item)
                            <option value="{{ $item->id_wisata }}" {{ old('lokasi_tujuan', $rute->lokasi_tujuan) == $item->id_wisata ? 'selected' : '' }}>{{ $item->nama_wisata }}</option>
                        @empty
                            <option value="">Tidak Ada Data Wisata</option>
                        @endforelse
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="any" class="form-control @error('jarak') is-invalid @enderror" id="jarak" name="jarak" value="{{ old('jarak', $rute->jarak) }}" required>
                    @error('jarak')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" step="any" class="form-control @error('waktu_tempuh') is-invalid @enderror" id="waktu_tempuh" name="waktu_tempuh" value="{{ old('waktu_tempuh', $rute->waktu_tempuh) }}" required>
                    @error('waktu_tempuh')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
